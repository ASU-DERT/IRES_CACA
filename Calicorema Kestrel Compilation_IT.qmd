...
## 1. Start from the raw list you read with read_csv()
##    (assumes you already have: data_list <- lapply(direct_links, read_csv(...)))

data_list_clean <- lapply(data_list, function(df) {
  # keep first 6 columns (datetime + 4 values + data type)
  df6 <- df[, 1:6]

  # give them standard names by position
  names(df6) <- c(
    "datetime",
    "temp_c",
    "rel_humidity",
  ...
  # drop data_type if it's always "point"
  df6 %>%
    filter(!is.na(datetime)) %>%  # just in case
    select(-data_type)
})
...
## 3. Extract logger IDs (7-digit number in filename)
logger_ids <- str_extract(filenames, "\\d{7}")

## 4. Name each element of the clean list by logger ID
names(data_list_clean) <- logger_ids

## 5. Combine into one big data frame
combined <- bind_rows(data_list_clean, .id = "logger_id")

## 6. (Optional) Nice human-readable datetime column
combined <- combined %>%
  mutate(
    datetime_friendly = format(datetime, "%B %d, %Y %I:%M %p")
  )

# 7. Separate the date and time into separate columns in the "combined" df 
combined<- combined |>
  mutate(
...
## NEW; need to check; Add in microsite and block information; need correct kestrel IDs and file. File on google drive is a test file heather used
df_loggers <- read.csv("/Users/isabeltorres/Documents/GitHub/IRES_CACA/Kestrel_IDs.csv", header = TRUE)
df_loggers$loggerID <- as.numeric(df_loggers$loggerID)
combined.logger.id<- combined |>
  left_join(df_loggers, join_by(loggerID))

# #. Remove any data recorded before the start date of the experiment
L1.combined.kestrel.data<- combined[combined[["date"]] >= "2023-07-04", ]  
```
