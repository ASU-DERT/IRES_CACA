---
title: "photosynthesis_CACA"
author: "Isabel Torres"
format: html
editor: visual
---
#IRES CACA Photosynthesis data analysis 
...
#Load in data frame and format for coding in R

```{r}
#|label: load in data frame and format for coding in R
df_CaCa_ps_sumdat=read.csv("https://www.dropbox.com/scl/fi/1ejch1jjq28dndz6gybvi/2023_cacaires_photo_sumdat_L2.csv?rlkey=xv10amoelqou3sjtpmvd2ylgc&st=tq6c7lkn&dl=1",header=TRUE) # reads in the photosynthesis file
...
```{r}
#|label: first look at the data after load and formatting 
...
```{r}
#|label: finding means and removing unnecessary lines
...
#Means of photosynthesis for one plant per day
```{r}
#|label: daily means for one plant only
...
```
#Figure: Boxplot of photosynthesis by treatment type
```{r}
...
```
#ANOVA of treatment differences
```{r}
...
```
#Exploring photosynthetic activity varibaility from stem area
```{r}
# Explore how photosynthesis varies with stem area

daily_plant_ps_means|>
  ggplot(aes(Area, mean_Photo)) +
...
daily_treatment_ps_means |>
 ...
 # scale_color_discrete(breaks=c("Control","SmallPulse","BigPulse")) 
  scale_color_manual(values=c("#E66101", "#1B9E77", "#56B4E9"), 
