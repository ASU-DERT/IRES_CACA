...
#IRES CACA 2023 #Data analysis for soil respiration data from the Calicorema Capitata experiment.
...
# plot Treatment.type means for each day
daily_Treatment.type_sr_means |>
  ggplot(aes(Date, efflux_by_Treatment.type, color = Treatment.type)) +
  geom_point(aes(shape = Treatment.type)) +
  geom_line(aes(linetype = Treatment.type)) +
  scale_color_manual(
    values = c(
      "Control"    = "#E66101",  # color-blind safe "vermillion"
      "SmallPulse" = "#1B9E77",  # "bluish green"
      "BigPulse"   = "#56B4E9"   # "sky blue"
    )
  ) +
  labs(
    x = "Date",
    y = expression(paste("Mean soil respiration (CO"[2], ")"))
  ) +
  theme_minimal() +
  geom_errorbar(
    aes(
      ymin = efflux_by_Treatment.type - se_efflux,
      ymax = efflux_by_Treatment.type + se_efflux
    ),
    linewidth = 0.4,
